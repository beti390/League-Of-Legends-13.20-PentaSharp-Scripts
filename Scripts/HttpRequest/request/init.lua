...
request = {
	...
	send = function(url, args, cb)
		local handle = curl.curl_easy_init()
		local multi_handle = curl.curl_multi_init()
		local p_still_running = ffi.new("int[1]")
		curl.curl_multi_add_handle(multi_handle, handle)
		local header_chunk
		local out_buffer
		...
		while true do
			local mc = curl.curl_multi_perform(multi_handle, p_still_running)
			if (mc ~= curl.CURLM_OK) then
				cb(false, request.error.unknown, "curl_multi_perform failed", mc)
				return
			end
			if (p_still_running[0] == 0) then
				break
			end
			coroutine.yield(url, args, cb)
		end

		local code = curl.CURLE_OK

		if (code ~= curl.CURLE_OK) then
			local num = tonumber(code)

			if (code_map[num]) then
				cb(false, code_map[num][1], code_map[num][2])
				return
			end

			cb(false, request.error.unknown, "Unknown error", num)
			return
		end
		...
		curl.curl_multi_remove_handle(multi_handle, handle)
    	curl.curl_multi_cleanup(multi_handle)
...
request.run = function()
	...
	if clock > next_run then
		next_run = clock + 0.050
		for i = 1, #active do
			local req = active[i]
			local ok, err = coroutine.resume(req.co, req.url, req.args, req.cb)
			if not ok then
				req.pending_remove = true
...
request.send = function(url, args, cb)
	table.insert(active, {
